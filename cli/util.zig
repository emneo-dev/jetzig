...
pub fn printSuccess(message: ?[]const u8) !void {
    try stdout.print(" " ++ icons.check ++ " {s}\n", .{message orelse ""});
}

/// Print a failure confirmation.
pub fn printFailure() !void {
    try stderr.print(" " ++ icons.cross ++ "\n", .{});
}

const PrintContext = enum { success, failure };
/// Print some output in with a given context to std{out,err}
...
pub fn execCommand(allocator: std.mem.Allocator, argv: []const []const u8) !void {
    ...
    for (argv) |arg| try stdout.print(" {s}", .{arg});
    try stdout.print("\n", .{});
...
pub fn runCommandInDir(allocator: std.mem.Allocator, argv: []const []const u8, dir: Dir, options: RunOptions) !void {
    ...
    var stdout_array = try std.ArrayListUnmanaged(u8).initCapacity(allocator, 0);
    var stderr_array = try std.ArrayListUnmanaged(u8).initCapacity(allocator, 0);
    errdefer {
        stdout_array.deinit(allocator);
        stderr_array.deinit(allocator);
    }

    try child.spawn();
    switch (options.output) {
        .capture => try collectOutput(child, allocator, &stdout_array, &stderr_array),
        .stream => {},
    }
    ...
    const result = std.process.Child.RunResult{
        .term = try child.wait(),
        .stdout = try stdout_array.toOwnedSlice(allocator),
        .stderr = try stderr_array.toOwnedSlice(allocator),
    };
...
fn collectOutput(
    ...
    stdout_array: *std.ArrayListUnmanaged(u8),
    stderr_array: *std.ArrayListUnmanaged(u8),
) !void {
    ...
    try writeFifoDataToArrayList(allocator, stdout_array, poller.reader(.stdout));
    try writeFifoDataToArrayList(allocator, stderr_array, poller.reader(.stderr));
}

// Borrowed from `std.process.Child.writeFifoDataToArrayList` - non-public but needed in our
// modified `collectOutput`
fn writeFifoDataToArrayList(allocator: std.mem.Allocator, list: *std.ArrayListUnmanaged(u8), reader: *std.Io.Reader) !void {
    var list_writer_buffer: [256]u8 = undefined;
    var list_writer = list.writer(allocator).adaptToNewApi(&list_writer_buffer);
    _ = try reader.streamRemaining(&list_writer.new_interface);
}
...
pub fn githubUrl(allocator: std.mem.Allocator) ![]const u8 {
    ...
    if (fetch_result.status != .ok) {
        try stderr.print("Error fetching from GitHub: {s}\n", .{url});
        return error.JetzigCommandError;
    }
