...
pub fn run(allocator: std.mem.Allocator, cwd: std.fs.Dir, args: [][]const u8, help: bool) !void {
    ...
    for (action_args) |arg| {
        if (parseAction(arg)) |action| {
            ...
            try util.stderr.print("Unexpected argument: {s}\n", .{arg});
            return error.JetzigCommandError;
        }
    }
    ...
    const file = dir.createFile(filename, .{ .exclusive = true }) catch |err| {
        switch (err) {
            error.PathAlreadyExists => {
                try util.stderr.print("Path already exists, skipping view creation: {s}\n", .{filename});
                return error.JetzigCommandError;
            },
            else => return err,
        }
    };
    try file.writeAll(util.strip(buf.written()));
    try file.writeAll("\n");
    file.close();
    const realpath = try dir.realpathAlloc(allocator, filename);
    defer allocator.free(realpath);
    try util.stdout.print("Generated view: {s}\n", .{realpath});
}
...
fn writeAction(allocator: std.mem.Allocator, writer: *std.Io.Writer, action: Action) !void {
...
// Write a view function to the output buffer.
fn writeTest(allocator: std.mem.Allocator, writer: *std.Io.Writer, name: []const u8, action: Action) !void {
...
fn writeStaticParams(allocator: std.mem.Allocator, actions: []Action, writer: *std.Io.Writer) !void {
    try writer.writeAll(
...
fn writeTemplate(allocator: std.mem.Allocator, cwd: std.fs.Dir, name: []const u8, action: Action) !void {
    ...
    const file = view_dir.createFile(filename, .{ .exclusive = true }) catch |err| {
        switch (err) {
            error.PathAlreadyExists => {
                try util.stderr.print("Path already exists, skipping template creation: {s}\n", .{filename});
                return;
            },
            else => return err,
        }
    ...
    defer allocator.free(realpath);
    try util.stdout.print("Generated template: {s}\n", .{realpath});
}
